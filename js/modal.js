...
function renderModal() {
  ...
  if (m.type === 'add-client') {
    ...
    body = `<div class="space-y-3">
      ...
      <div><label class="block text-xs font-medium text-gray-500 mb-1">Effort (capacity load)</label>
        <select id="m6" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0e7490]">
          <option value="1">Heavy — 1.0</option>
          <option value="0.5" selected>Medium — 0.5</option>
          <option value="0.25">Light — 0.25</option>
        </select>
      </div>
    ...
    body = `<div class="space-y-3">
      <p class="text-xs text-gray-500">These defaults apply per assignment. Integration effort is set individually per integration and isn't affected by this.</p>
      <div><label class="block text-xs font-medium text-gray-500 mb-1">Implementation Module (per distinct module owned)</label><input id="cw-module" type="number" step="0.05" min="0.05" max="10" value="${cw.module}" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0e7490]"/></div>
      <div><label class="block text-xs font-medium text-gray-500 mb-1">PMO / Master Assignee (per client)</label><input id="cw-pmo" type="number" step="0.05" min="0.05" max="10" value="${cw.pmo}" class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0e7490]"/></div>
